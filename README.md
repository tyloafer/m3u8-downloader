...
fork from: https://github.com/llychao/m3u8-downloader

golang 多线程下载直播流m3u8格式的视屏，跨平台。 你只需指定必要的 flag (`u`、`o`、`n`、`ht`等) 来运行, 工具就会自动帮你解析 M3U8 文件，并将 TS 片段下载下来合并成一个文件。
...
- s 是否允许不安全的请求, 默认 0
- sp 文件保存路径, 默认为当前路径
- f 自定义m3u8文件，避免m3u8文件的url被墙
- h 自定义host，配合 -f 使用，如果m3u8文件中ts片段连接中没有host，则可以通过 -h 指定实现
- k 自定义 aes key文件，避免 key 文件被墙
- m 合并指定目录下所有的ts文件
- p 指定socks/http代理
```
...
合并文件使用：./m3u8-downloader  -m ./test
指定m3u8及代理使用：./m3u8-downloader -f 1.m3u8 -h https://www.baidu.com -k aes.key -p sockes5://127.0.0.1:1080 -o test
```
